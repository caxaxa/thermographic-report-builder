...
def main() -> int:
    ...
    try:
        ...
        # Mesh is demoted to last-resort fallback due to:
        # - Large size (100MB+) causing bandwidth costs
        # - Redundant with source map (both use centrality scoring)
        # - Rarely triggered in practice (source map + camera reprojection + GPS cover most cases)
        mesh_path = None
        labeling_path = None
        ...
        if settings.enable_mesh_fallback:
            ...
            if mesh_path:
                ...
                mesh_backtracker = MeshBacktracker(
                    ...
                    nvm_path=nvm_path,            # Optional - for view_id -> image_name mapping
                    geo_converter=geo_converter,
                    camera_projector=camera_projector,
                    ortho_size=(uncropped_w, uncropped_h),
                )
        else:
            logger.info("Mesh fallback DISABLED (default) - skipping mesh artifact downloads to save bandwidth")
